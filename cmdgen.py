...
def persist_env_vars(config: dict) -> None:
    ...
    This ensures no duplicates are created by checking existing values.
    """
    if platform.system() == "Windows":
        ...
        for key, env_name in mapping.items():
            ...
            if current_value != value:
                ...
                console.print(f"[green]Environment variable {env_name} set permanently using setx.[/green]")
            else:
                console.print(f"[yellow]Environment variable {env_name} is already set to the current value, skipping.[/yellow]")
    else:
        ...
        try:
            # Read the rc file to check if the environment variables are already there
            with open(rc_file, "r") as f:
                existing_content = f.read()
            
            # Only append if the block is not already present
            if "# Added by CommandGen Setup" not in existing_content:
                with open(rc_file, "a") as f:
                    f.write(env_block)
                console.print(f"[green]Environment variables appended to {rc_file}.[/green]")
            else:
                console.print(f"[yellow]Environment variables are already appended in {rc_file}, skipping.[/yellow]")
...
def first_run_setup(config_file: str, gemini_api_key: str) -> str:
    ...
    and persist environment variables without creating duplicates.
    """
    ...
    # Check if GEMINI_API_KEY already exists before setting
    if "GEMINI_API_KEY" not in os.environ:
        os.environ["GEMINI_API_KEY"] = gemini_api_key

    selected_model = choose_model()

    # Capture system environment variables, avoiding duplicates
    config = {
...
def generate_command(query: str, os_name: str, shell: str = None) -> str:
    ...
    "- If the command involves actions that require elevated privileges, such as modifying system files, installing software, or moving files between restricted directories, ensure it is prefixed with sudo to grant the necessary administrative permissions.\n"
    "- If unsure whether sudo is required, assume it is needed for system modifications, installations, or root-owned file access.\n"
    "\nExamples:\n"
    "Example 1:\n"
    ...
    "Example 4:\n"
    "  Input: 'Install Python 3 using apt on Ubuntu.'\n"
